...
namespace AircraftEval {
    ...
    bool evaluate_off_design_aircraft(TS::Config& ip_config, const XPCSocket sock, const size_t num_f_evals,
        ...
        std::cout << "\n" << "Simulating Off-Design Aircraft " << std::to_string(num_f_evals) << "\n";
        ...
        double ISA_rho = 0;
        double ISA_visc = 0;
        AircraftModel::ISA(desired_h, ISA_T, ISA_a, ISA_P, ISA_rho, ISA_visc);
        ...
        for (size_t i = 0; i < 2; i++) {
            ...
            AircraftModel::compute_cg_loc_mass_offdesign(w_engine, w_fuel, mass_H2_design, ip_H2_Pfrac, num_pass_design,
                x_cg, mass_total, x_cg_nofuel, mass_nofuel, mass_payload, mass_JA1, mass_H2_net, op_num_pass, mass_violation,
                volume_violation);
